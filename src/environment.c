...
int environment_mut_variable(Environment *environment, const char *name,
                             const char *value) {
  if (environment == NULL) {
    return 1;
  }
  ...
  if (var_map_find(&environment->values, name, &tmp)) {
    var_map_put(&environment->values, strdup(name), strdup(value), free_string,
                free_string);
    return 0;
  }
...
void environment_create_variable(Environment *environment, const char *name,
                                 const char *value) {
  var_map_put(&environment->values, strdup(name), strdup(value), free_string,
              free_string);
}

void environment_create_function(Environment *environment, Z_String_View name,
                                 Statement_Function *fuction) {
  function_map_put(&environment->functions, name, fuction, NULL,
                   free_function_statement);
}

const char *environment_get_cstr(const Environment *environment,
                                 const char *name) {
  if (environment == NULL) {
...
Z_String_View environment_get(const Environment *environment,
                              Z_String_View name) {
...
Statement_Function *environment_get_function(const Environment *environment,
                                             Z_String_View name) {
  if (!environment) {
    return NULL;
  }
