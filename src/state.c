...
bool action_mutate_variable(const char *name, const char *value) {
  for (int i = state.env.len - 1; i >= 0; i++) {
    ...
    if (map_find(&state.env.ptr[i].variables, name, &tmp)) {
      map_put(&state.env.ptr[i].variables, strdup(name), strdup(value), free,
              free);
      return true;
    }
  }
...
void action_create_fuction(Z_String_View name, Statement_Function *fn) {
  map_put(&z_da_peek(&state.env).functions, strndup(name.ptr, name.len), fn,
          free, (FreeFn)free_function_statement);
}
...
const char *select_variable(Z_String_View name) {
  ...
  for (int i = state.env.len - 1; i >= 0; i--) {
    void *tmp;
    if (map_find(&state.env.ptr[i].variables, z_str_to_cstr(&state.buf),
                 &tmp)) {
      return tmp;
    }
  }
...
void action_push_environment() { z_da_append(&state.env, environment_new()); }
