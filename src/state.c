...
void action_create_fuction(Z_String_View name, Statement_Function *fn) {
  z_map_put(&z_da_peek(&state.env).functions, strndup(name.ptr, name.len), fn,
...
const char *select_variable(Z_String_View name) {
  ...
  for (int i = state.env.len - 1; i >= 0; i--) {
    void *tmp =
        z_map_get(&state.env.ptr[i].variables, z_str_to_cstr(&state.buf));
    if (tmp) {
      return tmp;
    }
  }
...
Statement_Function *select_function(Z_String_View name) {
  ...
  for (int i = state.env.len - 1; i >= 0; i--) {
    void *fn =
        z_map_get(&state.env.ptr[i].functions, z_str_to_cstr(&state.buf));
    if (fn) {
